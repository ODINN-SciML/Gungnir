...
def test_era5_file_generated():
    ...
    preprocessing_glaciers(
        rgi_ids, working_dir=working_dir, include_era5=True, years=years, test=True
    )

    era5_files = glob.glob(
        working_dir + "/**/climate_historical_monthly_ERA5.nc", recursive=True
    )
    assert len(era5_files) == len(
        rgi_ids
